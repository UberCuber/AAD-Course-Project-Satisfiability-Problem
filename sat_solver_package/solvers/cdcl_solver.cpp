...
struct Statistics {
    ...
    void print_csv() {
        // CSV format: result,time,depth,memory,decisions,backtracks,timeout
        // For CDCL: depth=_num_learned_clauses (clause learning depth), backtracks=_restarts
        std::cout << _result << ","
                  << (_complete_time - _start_time) << ","
                  << _num_learned_clauses << ","
                  << 0 << ","  // memory (not tracked in CDCL)
                  << _num_decisions << ","
                  << _restarts << ","
                  << 0 << std::endl;  // timeout flag
    }
};
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        if (to_log) {
            // Detailed output to file and console
